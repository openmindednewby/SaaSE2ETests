// Raw-translation-key detector — the shared scanner behind every portal's i18n guard spec.
//
// ── Generalised from the zygos original (2026-07-18) ───────────────────────────────────────────
//
// This detector was written for the zygos console and immediately found real production defects
// there. The bug class it catches is NOT zygos-specific — it is a property of the shared kit:
// `@dloizides/ui-layout`, `ui-tables`, `ui-forms` and `ui-feedback` resolve THEIR strings through
// the HOST app's `FM`, and `FM()` has no fallback. So every kit upgrade can add keys the host app
// has never heard of, in ANY app, and nothing in lint, typecheck, unit tests or the build says a
// word. The shared-UI wave that carried ui-forms@1.6.0 / ui-layout@1.11.0 / ui-tables@1.12.0
// across five portals is exactly that scenario, ~192 keys wide.
//
// Only the failure-message text was app-specific, so that is now a parameter
// (`describeRawKeys(screen, hits, localeFile)`) and everything else is unchanged and shared.
// `tests/zygos/zygos-i18n-scan.ts` re-exports this module, so the original zygos spec keeps
// working against the identical detector rather than a drifting copy.
//
// ── The bug class ─────────────────────────────────────────────────────────────────────────────
//
 ...
 *
 * `localeFile` points the reader at the exact dictionary to edit — it differs per portal
 * (`agora-web/src/localization/locales/en.json`, `erevna-web/...`, and so on), which is the only
 * app-specific thing left in this module.
 */
export function describeRawKeys(
  screen: string,
  hits: readonly RawKeyHit[],
  localeFile = 'the host app\'s src/localization/locales/en.json',
): string {
  ...
  return (
    ...
    `each of these is the literal key shown to the user. Add them to ` +
    `${localeFile}:\n${lines.join('\n')}`
  );
}
